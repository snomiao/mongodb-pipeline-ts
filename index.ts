...
import type { FieldArrayPath, FieldArrayPathValue, FieldPath, FieldPathValue, FieldValue } from "react-hook-form";
import { $flatten } from "./$flatten";
type NODOT<T extends string> = T extends `${infer K}.${infer _}` ? never : T;
...
export declare type $Set<TSchema extends Document> = {
  [P in keyof TSchema]?:
    | (TSchema[P] extends Record<string, any> ? $Set<UnwrapArrayValue<TSchema[P]>> : TSchema[P])
    | $Path<TSchema>;
} & Record<string, any>;
...
export function $pipeline<TSchema extends Document>(coll?: Collection<TSchema>, pipeline = [] as readonly Document[]) {
  ...
  return {
    ...
    setFlat<
      T extends {
        [k in FieldPath<TSchema>]: FieldValue<TSchema> | `$${FieldPath<TSchema>}`;
    ...
    setFlatten<T extends $Set<TSchema>>($set: T) {
      return $pipeline<$SetResult<TSchema, T>>(_coll, [...pipeline, { $set: $flatten($set) }]);
    },
    match(filter: Filter<TSchema>) {
      return $pipeline<TSchema>(_coll, [...pipeline, { $match: filter }]);
    },
    matchFlatten(filter: Filter<TSchema>) {
      return $pipeline<TSchema>(_coll, [...pipeline, { $match: $flatten(filter) }]);
    },
    unwind<Path extends FieldArrayPath<TSchema>>($unwind: `$${Path}`) {
      type Unwinded = { [k in Path]: NonNullable<FieldArrayPathValue<TSchema, Path>>[number] };
